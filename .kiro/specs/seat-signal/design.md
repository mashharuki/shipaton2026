...
- 混雑プロファイルの元データ整備作業そのもの（実走調査・ODPT からのデータセットビルドスクリプトの運用実行）— スキーマと格納形式のみ本スペックが定義
  - 2026-08-10 確認: 対象路線（中央線快速）の ODPT 時刻表データは「公共交通オープンデータチャレンジ限定ライセンス」（`api-challenge.odpt.org`、Challenge 2026 限定）でのみ提供され、一般の `api.odpt.org` 基本ライセンスとは別枠。Shipaton は東京公共交通オープンデータチャレンジと無関係のため、当該データをそのまま本番利用するのはライセンス上のリスクが高いと判断し採用を見送った（詳細根拠は project memory `odpt-timetable-challenge-license-2026-08.md`）。時刻表・混雑プロファイルとも当面は合成デモデータ＋フィードバック駆動の補正（`correction_stats`）で運用する方針を維持する。号車別混雑データは調査時点で一般公開APIとして存在しないことも既に確認済み（`docs/memo.md` データ調査報告）。
